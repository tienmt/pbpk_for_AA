...
FQ_Lu = 0.001
FQ_Li = 0.14
FQ_Ki = 0.155 #total - there is also an aterial flow rate 
FQ_T = 1 -( FQ_Li + FQ_Ki +FQ_P + FQ_Lu)
Q_Li <- Q_C*FQ_Li
Q_Ki <- Q_C*FQ_Ki
Q_T <- Q_C*FQ_T
Q_Lu <- Q_C*FQ_Lu
...
yini <- c(m_AA_Lu =0 , m_GA_Lu =0, 
          m_AA_AB = 0.0, m_GA_AB = 0.0, m_AA_VB = 0.0, m_GA_VB = 0.0,
...
PBPKmodelAA <- function(t,state,parameter){
  with(as.list(c(t,state,parameter)), {
    ...
    # units checked -> mg/h   
    dm_AA_Lu <- Q_Lu*(c_AA_AB - c_AA_Lu/pAA_Lu_B) -k_onAA_Lu*m_AA_Lu
    dm_GA_Lu <- Q_Lu*(c_GA_AB - c_GA_Lu/pGA_Lu_B) -k_onGA_Lu*m_GA_Lu
