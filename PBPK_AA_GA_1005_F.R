...
V_max_p450 = 0.5 * BW^0.7  # mg/h  from Sweenney paper, checked! (0.5 * BW^0.7 #)
...
pGA_LiB = 0.5 #0.63 # Have been calculated from the Doerge et al 2005 paper on TK in mice
pGA_KiB = .3
...
KM_EH = 100.0
# maximum rate of GA and AA conjugation with GSH (mg/(h BW^0.7))
Vmax_GA_GSH <- 20 * BW^0.7 #10*BW^0.7 # from the Sweeny ( Vmax_GC2 )  fitted   mg/h
...
K_FORM_AA_VAL = 6400 # 7500  # !fmol AA-val/mg globin per mM AA-hr

# Literature k_val_GA = 34000 fmol / (mg_globin * mM_GA * h)
#K_FORM_GA_VAL <- 34000 * Total_Globin_mg * 1e-12 # Result: ~0.0255 L/h
K_FORM_GA_VAL <- 59000 #34000 # fmol GA-val/mg globin per mM GA-hr
...
PBPKmodelAA <- function(t, state, parameter) {
  with(as.list(c(t, state, parameter)), {
    ...
    c_AA_Li_free <- c_AA_Li / pAA_LiB # the enzyme “sees” only the fraction considered available 
                                   #for reaction and transport, correcting for liver:blood partitioning.
    ...
    metAA_GSH  <- Vmax_AA_GSH * c_AA_Li_free * c_GSH_Li /((KMG1 + c_AA_Li_free) * (KMGG + c_GSH_Li))
    metAA_P450 <- V_max_p450 * c_AA_Li_free /(KM_p450 + c_AA_Li_free)
    ...
    c_GA_Li_free <- c_GA_Li / pGA_LiB # # blood-equivalent / free concentrations driving metabolism
    ...
    metGA_GSH <- Vmax_GA_GSH * c_GSH_Li * c_GA_Li_free / ((KMG2 + c_GA_Li_free) * (KMGG + c_GSH_Li))
    metGA_EH  <- V_max_EH * c_GA_Li_free / (KM_EH + c_GA_Li_free)
    ...
    volume_sum    <- V_AB + V_VB + V_mix + V_T + V_Li + V_Ki  # check if sum matches expected total blood/tissue volume
...
flux_ratio <- metAA_P450 / metAA_GSH
...
plot(yobs_urine$time, cumsum( tamtam ),type = 'l', ylab = 'aama', ylim = c(0,.15), xlab = 'time' ); grid()
...
#plot(out[,"time"], out[,"m_AA_Hb"], type="l")
...
times <- seq(from = 0, to = n_days * 24   , by = 0.1)
diet <- data.frame(var = "m_AA_dose", method = "add",   #  24* c(0:(n_days/2))
                   time = c(0 ),  value = 0.05 *BW ) #  0.5 *BW /1000   # dose of 0.05 microg/kg bw
out <- ode(y = yini, times = times, func = PBPKmodelAA, parms = params, events = list(data = diet))
...
plot(out[,'time'], 
     ...
     ylim = c(0, 3000)) # Adjusted ylim to typical range (0-150)
# Add reference points (ensure these values are also in pmol/g globin)
points(c(24, 120), c(51, 110), col="blue", cex = 1.5, pch = 17) ; grid()
...
plot(out[,'time'],  out[,'m_GA_Hb'], 
     ...
     ylim = c(0, 300)) # Adjusted ylim to typical range (0-150)
# Add reference points (ensure these values are also in pmol/g globin)
points(c(24, 120), c(51, 100), col="blue", cex = 1.5, pch = 17) ; grid()


#For the 1 mg/kg human experiment
n_days <- 1
...
diet <- data.frame(
  ...
  method = "add",
  time = c(0),
  value = 1 * BW   # 1 mg/kg * BW = mg total dose
)

out <- ode(y = yini, times = times, func = PBPKmodelAA,
           parms = params, events = list(data = diet))

par(mfrow = c(1,2), mar = c(4,4,1,1))

plot(out[,"time"], out[,"m_AA_Hb"],
     type = "l", lwd = 2,
     xlab = "Time (h)",
     ylab = "AAVal Hb adducts (fmol/mg globin)",
     main = "AA Hb adducts",
     ylim = c(0, 2300))

points(24, 914, col = "blue", pch = 17, cex = 1.5)
grid()

plot(out[,"time"], out[,"m_GA_Hb"],
     type = "l", lwd = 2,
     xlab = "Time (h)",
     ylab = "GAVal Hb adducts (fmol/mg globin)",
     main = "GA Hb adducts",
     ylim = c(0, 500))

points(24, 344, col = "blue", pch = 17, cex = 1.5)
grid()
...
plot(out[,"time"], out[,"m_AA_Hb"], type="l", lwd=2,
     xlab="Time (h)", ylab="AAVal (fmol/mg globin)",
     main="AA Hb (0.014 mg/kg)", ylim=c(0,50))
points(24, 8, col="blue", pch=17, cex=1.5); grid()

plot(out[,"time"], out[,"m_GA_Hb"], type="l", lwd=2,
     xlab="Time (h)", ylab="GAVal (fmol/mg globin)",
     main="GA Hb (0.014 mg/kg)", ylim=c(0,20))
points(24, 4, col="blue", pch=17, cex=1.5); grid()
...
plot(out[,"time"], out[,"m_AA_Hb"], type="l", lwd=2,
     xlab="Time (h)", ylab="AAVal",
     main="AA Hb (0.5 mg/kg)", ylim=c(0,1800))
points(24, 514, col="blue", pch=17, cex=1.5); grid()

plot(out[,"time"], out[,"m_GA_Hb"], type="l", lwd=2,
     xlab="Time (h)", ylab="GAVal",
     main="GA Hb (0.5 mg/kg)", ylim=c(0,300))
points(24, 186, col="blue", pch=17, cex=1.5); grid()
