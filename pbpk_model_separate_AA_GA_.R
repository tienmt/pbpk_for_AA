...
k_onAA_T = 0.028  #1/h     #  from Sweenney
k_onAA_B = 0.036  #1/h
k_onAA_Li = 0.71 #1/h
k_onAA_Ki = 0.13 #1/h
...
#  Formation Constants ( L / g_Hb h )
K_FORM_AA_VAL <- 1 
K_FORM_GA_VAL <- 34000 

# 3. Removal Constants (1/h)
K_REM_AA_VAL <- 0.00035 # 0.00035
K_REM_GA_VAL <- 0.00035 # 0.00035


hematocrit = 0.40 # fraction of blood volume that is red blood cells. Male: ~0.42–0.52  Female: ~0.37–0.47  
V_RBC = hematocrit * V_VB
k_deg_Hb = log(2)/(120*24)  # ≈ 0.00024 1/h RBC lifespan (~120 days)
...
yini <- c(m_AA_AB = 0.0, m_GA_AB = 0.0, m_AA_VB = 0.0, m_GA_VB = 0.0,
          ...
          m_AA_Li = 0.0, m_AAMA = 0.0,  m_GA_Li = 0.0, m_GAMA  = 0.0,       
...
PBPKmodelAA <- function(t, state, parameter) {
  with(as.list(c(t, state, parameter)), {
    ...
    dm_AA_Hb <- K_FORM_AA_VAL * c_AA_VB * V_RBC/MW_aa - k_deg_Hb * m_AA_Hb
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # -------------------------------------------------------------------------
    # Return: first element must be vector of derivatives
    # (in the same order as `state`)
    derivs <- c(
      dm_AA_AB,    dm_GA_AB,   
      dm_AA_VB,    dm_GA_VB,
